<?php
	...
	if(isset($_GET['action']) and $_GET['action']=="update" and !empty($_GET['npa']))
	{
		...
		$str="select * from pinjaman_uang where npa=".intval($_GET['npa']);
...
function cek()
{
	...
	if(pinjaman_pokok.value == "")
	{
		//alert("simp wajib harus diisi dulu");
		//return false;
		cek=cek+1;
		salah.push("pinjaman pokok harus diisi");
	}
	if(cek > 0)
	{
		alert(salah.join("\n"));
	...
	if(tanggal.value == "")
	{
		...
		//return false;
		cek=cek+1;
		salah.push("Harap Isi Semua Form Yang Tersedia");
	}
	if(cek > 0)
	{
		alert(salah.join("\n"));
...
<h1><b><font color=”#006400”>Form Pinjaman Uang</b></font></h1>
...
<a href="tampil_pinjaman.php"><button type="button"><img src="../../images/preview.png" alt="" title="" border="0" width="20px" height="20px">Preview</button ></a>
<a href="../../admin.php?menu=usp"  onclick="self.history.back()"><button type="button"><img src="../../images/exit3.png" alt="" title="" border="0" width="20px" height="20px">Exit</button></a>
	...
	<br>	
				...
				<td><label>NPA</label> </td>
					...
					<? $pinjamanxxxx = Array('usp','upk','unsos'); 
						//$i = 0;
...
<fieldset>
				...
				<tr>
					...
					<td>: <input name="pinjaman_pokok" id="pinjaman_pokok" type="text" size="30" onChange="hitungAngsuran()" value="<?php echo $pinjaman_pokok;?>" /></td>
				</tr>
				<tr>
