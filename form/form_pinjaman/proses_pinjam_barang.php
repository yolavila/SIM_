<?php
  include("../../db_config.php");//memanggil file koneksi database
	...
	$testing = mysql_query("select substring(no_pinjam,4) as no from pinjaman_barang order by no*1 DESC") or die(mysql_error());
  ...
  if (isset($_POST['caribtn'])){
	...
	{
	echo "<meta http-equiv=\"refresh\" content=\"0;url=http://localhost/skripsi/form/form_pinjaman/pinjaman_barang.php?id=$npa\">";
	}
	else
	{
		echo "<meta http-equiv=\"refresh\" content=\"0;url=http://localhost/skripsi/form/form_pinjaman/pinjaman_barang.php?ket=pasif\">";
	}
	}else{
	...
	$tgl_selesai=$_POST['tgl_selesai'];
  	...
  	if (isset($_POST['update'])){
				$exec=mysql_query("update pinjaman_barang set 
									npa = '$npa',
	...
	$exec = mysql_query("insert into pinjaman_barang ( no_pinjam, npa,tgl_pinjam, jenis_bayar, jenis_pinjaman, pinjaman_pokok, jasa, xangsuran, angsuran, tot_jasa,tot_angsuran,tgl_mulai, tgl_selesai) values ('$no_pinjam','$npa','$tgl_pinjam','$jenis_bayar','$jenis_pinjaman','$pinjaman_pokok','$jasa','$xangsuran','$angsuran','$tot_jasa','$tot_angsuran','$tgl_mulai','$tgl_selesai')") or die(mysql_error());
	for($i=1; $i<=$xangsuran; $i++){
	mysql_query("insert into angsuran_barang ( npa, no_pinjam, tgl_pinjam, angsuran, bayar) values ('$npa','$no_pinjam', DATE_ADD('$tgl_mulai', INTERVAL '$i' MONTH),'$angsuran','0')") or die(mysql_error());			
	}
	if ($exec){	
				echo '<meta http-equiv="refresh" content="0;url=http://localhost/skripsi/form/form_pinjaman/pinjaman_barang.php">';
			}else{
