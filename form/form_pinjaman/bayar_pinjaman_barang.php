<?php
session_start();
$filex= "bayar_pinjaman_barang.php";

?>

<?php
	require("../../db_config.php");
	$str="select * from angsuran_barang";
	...
	if (isset($_POST['sbtn'])){
		...
		$str = "SELECT * FROM angsuran_barang WHERE $opsi LIKE '%$search%'";
		//echo $str;
	}else{
		$str="select * from angsuran_barang";
	}
...
//	echo "<meta http-equiv=\"refresh\" content=\"0;url=http://localhost/skripsi/form/form_pinjaman/bayar_pinjaman_uang.php?id=$npa\">";
	echo "<meta http-equiv=\"refresh\" content=\"0;url=http://localhost/skripsi/form/form_pinjaman/bayar_pinjaman_barang.php?id=$no_pinjam\">";
	}
	if(isset($_POST['save']))
	{
		$x=$_POST['xangsuran'];
		...
		for($i=1;$i<=$x;$i++)
		{
			...
			if($byr == 1)
			{
				$qbyr = mysql_query("update angsuran_barang set bayar = '1' where id = '".$_POST['idbayar_'.$i.'']."'") or die(mysql_error());
				//echo $_POST['bayar_'.$i.''];
				if($qbyr)
				{
					$cek = $cek + 0;
			...
			{
				$qbyr = mysql_query("update angsuran_barang set bayar = '0' where id = '".$_POST['idbayar_'.$i.'']."'") or die(mysql_error());
				//echo $_POST['bayar_'.$i.''];
				if($qbyr)
				{
					$cek = $cek + 0;
...
//	$npax=$_GET['id'];
	...
	$cariagt = "SELECT a.npa,a.nama, a.instansi, p.no_pinjam, p.pinjaman_pokok, p.jasa, p.xangsuran, p.angsuran, p.tot_jasa, p.tot_angsuran, p.tgl_mulai, p.tgl_selesai FROM pinjaman_barang p inner join anggota a on a.npa=p.npa AND p.no_pinjam='$no_pinjamx' LIMIT 0,1";
	...
	$itung	 = "SELECT count(*) as jml FROM `pinjaman_barang` WHERE npa='$npax'";
...
<a href="tampil_angsuran_barang.php"><button type="button"><img src="../../images/preview.png" alt="" title="" border="0" width="20px" height="20px">Preview</button ></a>
...
<fieldset>
	...
	if(isset($no_pinjamx))
	{
	$cariang = "Select * from angsuran_barang where no_pinjam='$no_pinjamx'";
	...
	$qsisa = mysql_query("SELECT SUM(angsuran) AS tot_angsuran FROM angsuran_barang where no_pinjam = '$no_pinjamx' and bayar = '0'")or die(mysql_error());
