...
<form method="post" name="formOut" action="proses_anggota_out.php" id="formOut" onSubmit="return cek();">	
<h1><b><font color=”#006400”>Form Anggota Keluar</b></font></h1>
<div class="button"> 
...
<a href="tampil_out.php"><button type="button"><img src="../../images/preview.png" alt="" title="" border="0" width="20px" height="20px">Preview</button ></a>
...
<fieldset>
			...
			<td>: <input id="semua" name="jumlah" type="text" value="<?php
	$semua_simpan = mysql_query("SELECT SUM(simp_wajib)+SUM(simp_pokok)+SUM(simp_sukarela) AS jumlah FROM simpanan where tipe='s' AND npa = '$npax'")or die( mysql_error());
	$simpan_semua = mysql_fetch_assoc($semua_simpan);
	$semua_ambil = mysql_query("SELECT SUM(simp_wajib)+SUM(simp_pokok)+SUM(simp_sukarela) AS jumlah FROM simpanan where tipe='a' AND npa = '$npax'")or die( mysql_error());
	$ambil_semua = mysql_fetch_assoc($semua_ambil);
	echo ($simpan_semua['jumlah']-$ambil_semua['jumlah']);
	?>"/> <td>
	</tr>
				
	<tr>
					<td><label>Keterangan</label> </td>
					<td>: <input name="ket" type="text" value="Keluar dari Keanggotaan"/> </td>
				</tr>
			</fieldset>	
</span>
