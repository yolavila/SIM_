<?php
	...
	if (isset($_POST['sbtn'])){
		...
		{
			$str="SELECT a.nama,s.* FROM `simpanan` s
					Left Join anggota a on s.npa=a.npa WHERE s.tipe='s' ";
			$hapus = TRUE;
		}
		if($_SESSION['level'] == 4 ) //|| Tampilan Pada Anggota
		{
			...
			$str="SELECT a.nama,s.* FROM `simpanan` s
					Left Join anggota a on s.npa=a.npa WHERE s.tipe='s' AND s.npa = '$npa'";
			$hapus = FALSE;
		}
...
<fieldset>
					<h1><b><font color=”#006400”>Tampil Simpanan</b></font></h1>
					<div id="contact_form">
					<div id="main">
					</div>
...
<tr><td width="50%">
	...
	<thead>
	<tr bgcolor="#3CB371">
	<th width="60">No.Simp</th><th width="60">NPA</th><th width="200">Nama</th><th width="70">Tanggal</th><th width="80">Simp.Wajib</th><th width="80">Simp.Pokok</th><th width="80">Simp.Sukarela</th><th width="250">Keterangan</th><?php if($hapus){ ?> <th width="50">Delete</th><?php } ?>
	</tr> 
	</thead>
	...
	$item_per_page = 20;
	$sql = $str;
	$query         = mysql_query( $sql ) or die(mysql_error()); //ganti
	...
	//paging( $sql,$item_per_page );     GANTIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII
	$batas = ($page - 1) * $item_per_page;
	$sql_ = $str." ORDER BY substring(no_simpanan,4)*1 ASC LIMIT $batas, $item_per_page";
	...
	$query_ = mysql_query( $sql_ ) or die(mysql_error()); //ganti
	...
	<tr class="gradeA">
				...
				<td><a href="form_simpanan.php?action=update&npa=<?php echo $ret['npa'];?>" style="text-decoration:none"><?php echo $ret['nama']; ?></td>
				...
				<? 
				$data = $ret['simp_pokok'];
				echo " ".number_format($data,0, ',', '.'); //nol 2 blakang=($data,2);
				?></td><td>
				<? 
	...
	<?php if($hapus){ ?>			
	<td align="center"><a href="tampil_simpanan.php?action=delete&no_simpanan=<?php echo $ret['no_simpanan'];?>" onclick="return confirm('Are you sure you want to delete?')"><img src="../../images/trash.png"  border="0"></a></td>
	</tr>
	<?php } ?>
