...
function Header()
{
    ...
    $this->Cell(150);
    // Title
    $this->Cell(30,10,'Laporan Return Penjulan',0,0,'C');
...
$pdf->Cell(30, 12, 'NPA', 1, 0, 'C', 1);
		...
		$pdf->Cell(30, 12, 'INSTANSI', 1, 0, 'C', 1);
//		$pdf->Cell(30, 12, 'ID BARANG', 1, 0, 'C', 1);
		...
		$pdf->Cell(40, 12, 'TANGGAL RETURN', 1, 0, 'C', 1);
...
if($blnLaporan != "")
{
$result=mysql_query("SELECT a.nama as namang, a.instansi, mb.nama as nambar, mb.harga_jual,s.* FROM `return_penjualan` s Left Join anggota a on s.npa=a.npa left join master_barang mb on s.id_barang=mb.id_barang where month(tgl) = ".$_POST['bulan']." AND year(tgl) = '$thnLaporan' ORDER BY id_return");
}
else
{
//echo "SELECT p.npa npa, p.id_jual idjual, p.njb njb, p.id_barang id, mb.nama nm, p.quatity qty, mb.harga_jual harga, p.jumlah jml FROM ".$table." p LEFT JOIN master_barang mb ON mb.id_barang = p.id_barang where year(tgl_jual) = '$thnLaporan' ORDER BY id_return";
$result=mysql_query("SELECT a.nama as namang, a.instansi, mb.nama as nambar, mb.harga_jual,s.* FROM `return_penjualan` s Left Join anggota a on s.npa=a.npa left join master_barang mb on s.id_barang=mb.id_barang where year(tgl) = '$thnLaporan' ORDER BY id_return");
}
//initialize counter
$i = 0;
...
//		$pdf->Cell(30, 12, 'ID BARANG', 1, 0, 'C', 1);
	...
	$pdf->Cell(30, 6, $instansi, 1, 0, 'L', 1);
  // 	$pdf->Cell(30, 6, $id_barang, 1, 0, 'L', 1);
	$pdf->Cell(70, 6, $nama_barang, 1, 0, 'L', 1);
	$pdf->Cell(30, 6, $quatity, 1, 0, 'L', 1);
	$pdf->Cell(30, 6, $jumlah, 1, 0, 'L', 1);
	$pdf->Cell(40, 6, $tgl, 1, 0, 'L', 1);
