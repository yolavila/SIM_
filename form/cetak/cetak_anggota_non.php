...
function Header()
{
    ...
    $this->Cell(150);
    // Title
    $this->Cell(30,10,'Laporan Anggota Non Aktif',0,0,'C');
...
$pdf->Cell(60, 12, 'INSTANSI', 1, 0, 'C', 1);
$pdf->Cell(25, 12, 'KOTA', 1, 0, 'C', 1);
$pdf->Cell(30, 12, 'JK', 1, 0, 'C', 1);
...
if($blnLaporan != "")
{
$result=mysql_query("select npa, nama, alamat, instansi, kota, tgl_masuk, jk, from ".$table." where month(tgl_masuk) = '$blnLaporan' AND year(tgl_masuk) = '$thnLaporan' AND tipe='p' ORDER BY npa ");
}
else
{
...
$result=mysql_query("select npa, nama, alamat, instansi, kota, tgl_masuk, jk from ".$table." where year(tgl_masuk) = '$thnLaporan' AND tipe='p' ORDER BY npa");
}
//initialize counter
$i = 0;
...
while($row = mysql_fetch_array($result))
{
	...
	$pdf->Cell(60, 6, $instansi, 1, 0, 'L', 1);
	$pdf->Cell(25, 6, $kota, 1, 0, 'L', 1);
	$pdf->Cell(30, 6, $jk, 1, 0, 'L', 1);
	...
	if($brs > $jmlBaris)
	{
		...
		$str="select a.nama from pengurus p left join anggota a on a.npa = p.npa AND p.jabatan='ketua'";
		$nm = mysql_query($str) or die(mysql_error());
		$nama = mysql_fetch_row($nm);
