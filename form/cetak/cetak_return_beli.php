...
function Header()
{
  ...
  //  $this->Cell(30,10,$jdlLaporan,0,0,'C');
	  $this->Cell(30,10,'Laporan Return Pembelian',0,0,'C');
	$this->Ln(3);
...
if($blnLaporan != "")
{
$result=mysql_query("SELECT m.nama as nambar, m.harga_jual, p.nama as namsup, s.* FROM ".$table." s Left Join master_barang m  on s.id_barang=m.id_barang left join suplier p  on p.kode_suplier=s.kode_suplier where month(tgl) = ".$_POST['bulan']." AND year(tgl) = '$thnLaporan' ORDER BY id_return");
}
else
{
...
$result=mysql_query("SELECT m.nama as nambar, m.harga_jual, p.nama as namsup, s.* FROM ".$table." s Left Join master_barang m  on s.id_barang=m.id_barang left join suplier p  on p.kode_suplier=s.kode_suplier where year(tgl) = '$thnLaporan' ORDER BY id_return");
}
//initialize counter
$i = 0;
...
while($row = mysql_fetch_array($result))
{
    ...
    if ($i == $max)
    {
        ...
        $pdf->SetY($y_axis_initial);
        $pdf->SetX(40);
        $pdf->Cell(30, 12, 'ID.RETURN', 1, 0, 'C', 1);
		$pdf->Cell(30, 12, 'ID.BARANG', 1, 0, 'C', 1);
		$pdf->Cell(60, 12, 'SUPLIER', 1, 0, 'C', 1);
		$pdf->Cell(60, 12, 'NAMA BARANG', 1, 0, 'C', 1);
		$pdf->Cell(20, 12, 'QUATITY', 1, 0, 'C', 1);
    ...
    $pdf->SetY($y_axis);
    $pdf->SetX(40);
    $pdf->Cell(30, 6, $no_return, 1, 0, 'L', 1);
	$pdf->Cell(30, 6, $id_barang, 1, 0, 'L', 1);
	$pdf->Cell(60, 6, $suplier, 1, 0, 'L', 1);
