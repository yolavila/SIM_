...
<?php
	...
	$cariagt = "SELECT a.kode_suplier kodex, a.nama namax,p.* FROM `pembelian` p
				LEFT JOIN suplier a ON a.kode_suplier=p.kode_suplier WHERE npb='$npbx' LIMIT 0,1";
	$qcari	 = mysql_query($cariagt) or die(mysql_error());
	$datacari= mysql_fetch_assoc($qcari); 
	$namax	 = $datacari['namax'];
	$kdx 	 = $datacari['kodex'];
	$itung	 = "SELECT count(*) as jml FROM pembelian WHERE npb='$npbx'";
...
<form method="post" name="formReturn" action="proses_return.php" id="formReturn"  onSubmit="return cek();">	

<h1><b><font color=”#006400”>Return Pembelian Barang</b></font></h1>
<div class="button"> 
...
<a href="tampil_return_pembelian.php"><button type="button"><img src="../../images/preview.png" alt="" title="" border="0" width="20px" height="20px">Preview</button ></a>
<a href="../../admin.php?menu=toko"  onclick="self.history.back()"><button type="button"><img src="../../images/exit3.png" alt="" title="" border="0" width="20px" height="20px">Exit</button></a>
               ...
               <table>
				<tr>
					...
					<td>: <input name="struk" id="struk" type="text" size="30" value="<?php echo $npbx;?>"/></td>
					<td><input type="submit" name="caribtn" id="submit" value="Cari"> </td>
				</tr>	
				<tr>
					<input name="kdsuplier" id="kdsuplier" type="hidden" value="<?php echo $kdx;?>" />
					<td><label>Suplier</label></td>
					<td>: <input name="suplier" id="suplier" type="text" size="30"value="<?php echo $namax;?>" />
				...
				<tr>
					...
					<td>: <input type="text" name="tgl" id="tanggal"  size="30" value="<?php echo date("j/n/Y"); ?>" /></td>
				</tr>
...
<table width="100%" border="1" cellpadding="5" cellspacing="0" >
				<?php
				$qry = mysql_query("SELECT p.id_beli idbeli, p.npb npb, p.id_barang id, mb.nama nm, p.quatity qty, mb.harga_beli harga, p.jumlah jml FROM `pembelian` p
						LEFT JOIN master_barang mb ON mb.id_barang = p.id_barang
						WHERE p.npb = '$npbx'") or die(mysql_error());
				$i=0;
				...
				<td width="5px" align="center"><a href="proses_return.php?action=return&id_beli=<?php echo $npbx;?>&id_barang=<?php echo $dataxx['id'];?>" onclick="return confirm('Anda Yakin Ingin Mengembalikan barang?')"><img src="../../images/undo.png"  border="0"></a></td>
				</tr>
