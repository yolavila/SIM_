<?php
	...
	if (isset($_POST['sbtn'])){
		...
		$str = "SELECT * FROM penjualan WHERE $opsi LIKE '%$search%'";
		//echo $str;
	}else{
		$str="select distinct a.nama, p.npa, a.instansi, count(px.npa) hitung
from penjualan p
LEFT JOIN anggota a ON p.npa=a.npa
LEFT JOIN penjualan px ON p.npa=px.npa
GROUP BY p.npa";
	}
...
<h1><b><font color=”#006400”>Reward Anggota</b></font></h1>
...
<input type="text" name="search" id="search">
  <input type="submit" name="sbtn" id="submit" value="Cari"> 
	...
	$jmldata=mysql_num_rows($hasil2) or die(mysql_error());
...
<thead>	
<tr bgcolor="#3CB371">
<th width="40">Npa</th><th width="150">Nama</th></th><th width="120">Instansi</th><th width="30">Beli (Kali)</th>
</tr> 
</thead>
	...
	$sql_ = $str." ORDER BY hitung DESC LIMIT $batas, $item_per_page";
	...
	<tr class="gradeA">
			...
			<td>
			<?php
			$kali = mysql_query("SELECT * FROM penjualan where npa = '".$ret['npa']."'");
			$result = mysql_num_rows($kali);
			echo $result;
			?>
	</td>
